                                ...
                                <tr class="hover:bg-white/20 transition-colors">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <div class="w-10 h-10 bg-gradient-to-br from-violet-400 to-violet-600 rounded-full flex items-center justify-center text-white font-semibold text-sm shadow-lg">
                                                <?= strtoupper(substr($user['name'], 0, 1)) ?>
                                            </div>
                                            <span class="text-sm font-medium text-gray-900"><?= $user['name'] ?></span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-700"><?= $user['nim_nip'] ?></td>
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        $roleClass = [
                                            ...
                                            'kepala_lab' => 'bg-amber-100 text-amber-700',
                                            'user' => 'bg-emerald-100 text-emerald-700'
                                        ];
                                        ?>
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium <?= $roleClass[$user['role']] ?>">
                                            <?= ucfirst(str_replace('_', ' ', $user['role'])) ?>
                                        </span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-2">
                                            <a href="<?= BASE_URL ?>user/edit/<?= $user['id'] ?>" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/30 text-gray-600 hover:text-amber-600 transition-colors">
